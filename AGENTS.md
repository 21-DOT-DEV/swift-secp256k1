...
- **XCFramework test manifest**: `Scripts/XCFrameworkTest/Package.swift` is a dedicated manifest the Bitrise "TEST XCFRAMEWORK" step copies over the root `Package.swift`, so `swift test` exercises the just-built `P256K.xcframework` binary (via `Tests/XCFrameworkTests`, which has no target in the main manifest). It must keep the `libsecp256k1` C target — the framework's swiftinterface imports it — with all module defines unconditional, matching the all-modules Xcode build. Do not delete.
