...
struct SecureBytes {
    ...
    @usableFromInline
    init(count: Int) {
        self.backing = Self.Backing.create(randomBytes: count)
    }
...
extension SecureBytes: Collection {
    ...
    subscript(_ index: Index) -> UInt8 {
        get {
            self.backing[offset: index.offset]
        }
        set {
...
extension SecureBytes.Index: Strideable {
    func advanced(by n: Int) -> SecureBytes.Index {
        SecureBytes.Index(offset: self.offset + n)
    }
...
extension SecureBytes {
    ...
    internal class Backing: ManagedBuffer<BackingHeader, UInt8> {
        ...
        class func create(capacity: Int) -> Backing {
            ...
            return Self.create(minimumCapacity: capacity, makingHeaderWith: { _ in BackingHeader(count: 0, capacity: capacity) }) as! Backing
        }

        @usableFromInline
        class func create(copying original: Backing) -> Backing {
            Self.create(bytes: original)
        }

        @inlinable
        class func create<D: ContiguousBytes>(bytes: D) -> Backing {
            bytes.withUnsafeBytes { bytesPtr in
                let backing = Self.create(capacity: bytesPtr.count)
        ...
        class func create(randomBytes: Int) -> Backing {
            let backing = Self.create(capacity: randomBytes)
            backing._withVeryUnsafeMutableBytes { targetPtr in
