...
struct AsymmetricTestSuite {
    ...
    func testPrivateKey() {
        #expect((try? secp256k1.Signing.PrivateKey()) != nil)
    }
    ...
    func testCompressedPublicKey() {
        ...
        #expect(privateKey.publicKey.dataRepresentation.count == secp256k1.Format.compressed.length, "PublicKey length mismatch for compressed format")
    }
    ...
    func testUncompressedPublicKey() {
        ...
        #expect(privateKey.publicKey.dataRepresentation.count == secp256k1.Format.uncompressed.length, "PublicKey length mismatch for uncompressed format")
    }
