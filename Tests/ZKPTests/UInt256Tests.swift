...
//===----------------------------------------------------------------------===//
    ...
    final class SIMDWordsIntegerTests: Base {
        static func main() {
                ...
                testSuite.test("ModularArithmetic", testCase.testModularArithmetic)
    ...
    extension SIMDWordsIntegerTests {
        func testModularArithmetic() {
            // addMod: 0 + 0 mod 2 = 0
            expectEqual(UInt256.zero, UInt256.zero.addMod(.zero, modulus: 2))

            // addMod: 1 + 1 mod 3 = 2 (no-overflow, sum < modulus)
            expectEqual(2, (1 as UInt256).addMod(1, modulus: 3))

            // addMod: 2 + 2 mod 3 = 1 (no-overflow, sum >= modulus, subtract once)
            expectEqual(1, (2 as UInt256).addMod(2, modulus: 3))

            // addMod: small values, sum < modulus
            expectEqual(5, (2 as UInt256).addMod(3, modulus: 100))

            // addMod: small values, sum >= modulus
            expectEqual(1, (98 as UInt256).addMod(3, modulus: 100))

            // addMod: overflow path — (2^255) + (2^255) = 2^256 (overflows), mod 3
            // 2^256 mod 3: 2^1=2, 2^2=1 (mod 3, period 2), 256 even → 2^256 ≡ 1 (mod 3)
            let half: UInt256 = 1 << 255
            expectEqual(1, half.addMod(half, modulus: 3))

            // mulMod: 0 * anything = 0
            expectEqual(UInt256.zero, UInt256.zero.mulMod(UInt256.max, modulus: 7))

            // mulMod: identity
            expectEqual(5, (5 as UInt256).mulMod(1, modulus: 7))

            // mulMod: 3 * 4 mod 7 = 5
            expectEqual(5, (3 as UInt256).mulMod(4, modulus: 7))

            // mulMod: large operands — secp256k1 field prime (128-bit value, fits in UInt256)
            // x = floor(p/2), y = 2  =>  x * y = p - 1 (no overflow in ref calc)
            let p: UInt256 = 0xFFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFE_FFFF_FC2F
            let x: UInt256 = p >> 1 // (p-1)/2 for odd p
            let ref: UInt256 = x &* 2 // = p - 1, stays < UInt256.max
            expectEqual(ref % p, x.mulMod(2, modulus: p))
        }
    }
